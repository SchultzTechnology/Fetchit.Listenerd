...
echo "    --broker-port 443 \\"
echo "    --topic-sub \"fetchit/commands/#\" \\"
echo "    --topic-pub \"fetchit/status/\" \\"
echo "    [--otel-token \"<signoz-auth-token>\"]"
echo "  (--broker-port = MQTT broker TCP port; SIP port is set in appsettings.json)"
echo "  Note: 'listenerd' must be run with sudo — the config DB is owned by root."
echo ""
